...
class Material:
    ...
    def inLight(self, polAng = 0):
        ...
        interact2d = []
        interact2d.append(interact[0])
        interact2d.append(interact[1])
        #self.interact2d = interact2d #uncomment for testing
        
        '''accounts for rotation from half wave plate: half wave plate is at theta/2
        going in the half wave plate changes the polarization of the light from 100 to
        whatever angle is specified. On the way out, the interaction might be a bit more
        complicated as the diagonal of the delta n are not the same.'''
        
        s = np.sin(np.deg2rad(-1*polAng/2))
        c = np.cos(np.deg2rad(-1*polAng/2))
        
        ReflMat        = [[-1,0],[0,1]]
        halfWaveJones  = [[c**2-s**2,2*c*s],[2*c*s, -c**2+s**2]]
        beamSplitter   = [[1,0],[0, 0]]  
        
        interact1 = np.matmul(ReflMat,interact2d)
        interact2 = np.matmul(halfWaveJones,interact1) 
        self.interact2 = interact2
        interact3 = np.matmul(beamSplitter, interact2)        
        
        #amplitude of reflected electric field from dn:
        self.amp1      = np.sqrt(interact3[0]**2)  #+ interact3[1]**2 ) 
